...
export interface TimelineVectorSnapshot {
  readonly absoluteTicks: number;
  readonly bars: number;
  readonly primary: number;
  readonly secondary: number;
  readonly ticks: number;
  readonly timeSignature: TimeSignatureSnapshot;
}

export class TimelineVector extends Simple<TimelineVectorSnapshot> {
  readonly absoluteTicks: number;
  readonly bars: number;
  readonly primary: number;
  readonly secondary: number;
  ...
  toSnapshot() {
    const { absoluteTicks, bars, primary, secondary, ticks } = this;
    const timeSignature = this.timeSignature.toSnapshot();
    return { absoluteTicks, bars, primary, secondary, ticks, timeSignature };
  }

  fromSnapshot(snapshot: TimelineVectorSnapshot) {
    const { bars, primary, secondary, ticks, timeSignature: ts } = snapshot;
    const timeSignature = new TimeSignature(ts.numerator, ts.denominator);
